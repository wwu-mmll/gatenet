...
from revision_analyses.fate import MLP, FATE, SupervisedModel
...
def train_transformer(df_train, df_valid, ds_kwargs):
    ...
    fate = FATE()
    mlp = MLP(dim_output=len(ds_kwargs['vocab']))
    model = SupervisedModel(encoder=fate, pred_head=mlp, n_marker=ds_train.fcs_dfs[0].shape[1])
    model = model.cuda()
    ...
    progress_bar = tqdm(range(300))  # tqdm(range(2))
    for epoch in progress_bar:
        loss = 0.
        optim.zero_grad()
        for batch_idx, (fcs, label) in enumerate(dl_train):
            fcs, label = fcs.cuda(), label.cuda()
            marker_idx = torch.arange(fcs.shape[2]).repeat((fcs.shape[1], 1))
            label_pred = softmax(model(fcs[0], marker_idx)[0])
            loss += loss_func(label_pred, label[0])
            # label_pred = label_pred.transpose(0, 1)
            # loss += loss_func(label_pred[None], label[0][None])
            # print(loss)
            # tqdm.write(f'Loss: {loss.cpu().item()}')
            if (batch_idx + 1) % 8 == 0 and batch_idx != len(dl_train) - 1:
                loss.backward()
                optim.step()
                scheduler.step()
                loss = 0.
    ...
    with torch.no_grad():
        for fps, (fcs, label) in zip(df_valid.fcs, dl_valid):
            ...
            results.update({Path(fps).stem: get_metrics(label_pred, label[0], len(ds_kwargs['vocab']))})
...
if __name__ == '__main__':
    ...
    for probe in ['L', 'B']:
        ...
        print(res)
        res_all.append(res)
    pd.concat(res_all).to_csv('/home/lfisch/Projects/gatenet_old/data/results/ms/fate_results.csv')
