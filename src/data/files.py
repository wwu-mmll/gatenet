...
def get_files_df(fcs_dir: str, label_dir: str = None, gates_dir: str = None, file_idxs: list = None,
                 substr: str = None, gate_substr: str = None, gate_idx: list = None) -> pd.DataFrame:
    datapaths = {}
    ...
    if label_dir is not None:
        datapaths.update({'labels': filepaths(label_dir, substr, '.csv').iloc[file_idxs]})
    if gates_dir is not None:
        substr = substr if substr is None else [substr, gate_substr]
        datapaths.update({'gates': filepaths(gates_dir, substr, '.csv').iloc[file_idxs]})
