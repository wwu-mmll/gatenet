from fastai.basics import nn, torch, F, Module, Tensor


class DenseNet(nn.Module):
    def __init__(self, hparams: dict):
        super(DenseNet, self).__init__()
        layers_sizes = hparams['layers_sizes']
        self.fc1 = nn.Linear(in_features=hparams['n_markers'], out_features=layers_sizes[0])
        self.fc2 = nn.Linear(in_features=layers_sizes[0], out_features=layers_sizes[1])
        self.fc3 = nn.Linear(in_features=layers_sizes[1], out_features=layers_sizes[2])
        self.activation = F.relu if layers_sizes[0] == 128 else nn.Softplus()
        self.out = nn.Linear(in_features=layers_sizes[2], out_features=hparams['n_gates'], bias=True)
        self.softmax = nn.Softmax(dim=1)

    def forward(self, x: Tensor, x_c: Tensor, single_sample=False):
