...
from .model import GateNet, DenseNet
...
def train(df_train: DataFrame, df_valid: DataFrame, hparams: dict,
    ...
    print(sum(p.numel() for p in learn.model.parameters()))
    if 'iters' in hparams:
...
def get_dls(df_train: DataFrame, df_valid: DataFrame, ds_kwargs: dict, dl_kwargs: dict) -> DataLoaders:
    if ds_kwargs is None:
        ds_kwargs = {}
    if 'valid_gates' in df_valid:
        df_valid = df_valid.copy()
        df_valid['gates'] = df_valid.valid_gates
        # df_valid['labels'] = df_valid.valid_gates
...
def get_learner(dls: DataLoaders, hparams: dict, metrics: dict = METRICS):
    model = DenseNet(hparams) if 'layers_sizes' in hparams else GateNet(hparams)
